...
class Main(object):
	...
	def load_data(self):
		...
		for (sub, rel), obj in self.sr2o.items():
			self.triples['train'].append({'triple':(sub, rel, -1), 'label': self.sr2o[(sub, rel)], 'sub_samp': 1})  # obj
	...
	def evaluate(self, split, epoch):
		...
		self.logger.info(left_results)
		self.logger.info(right_results)
		...
		with torch.no_grad():
			...
			for step, batch in enumerate(train_iter):
				...
				for i in range(pred.shape[0]):
					...
					hit_in_100_lst.append(1 if _filter <= 100 else 0)

				if step % 100 == 0:
					self.logger.info('[{}, {} Step {}]\t{}'.format(split.title(), mode.title(), step, self.p.name))
		import pandas as pd
		df = pd.DataFrame({'all':hit_in_100_lst})
		df.to_csv(f'./{split}_{mode}_spider_input.csv', index=False)
		return results
	...
	def run_epoch(self, epoch, val_mrr = 0):
		...
		for step, batch in enumerate(train_iter):
			...
			# self.logger.info('Label sum : {:.1}'.format(label.sum()))
			# tmp = torch.gt(label[0],0.0002)
			# print(tmp.sum())
			pred	= self.model.forward(sub, rel)
	...
	def fit(self):
		self.best_val_mrr, self.best_val, self.best_test, self.best_test_mrr, self.best_epoch = 0., {}, {}, 0., 0.  # 这个顺序看着不爽
		val_mrr = 0
		save_path = os.path.join('./models', self.p.ckpt,self.p.max_epochs)
		...
		for epoch in range(self.p.max_epochs):
			...
			if val_results['mrr'] > self.best_val_mrr:   # 这里可以保存模型
...
if __name__ == "__main__":
	...
	parser.add_argument('-data',            dest="dataset",         default="PharmKG",            help='Dataset to use') # 'FB15k-237'
	parser.add_argument("-name",            default='PharmKG',					help="Set filename for saving or restoring models")
	parser.add_argument("-ckpt",            default="pharmkg",                  help="Set previous model")

	parser.add_argument('-batch',           dest="batch_size",      default=128,    type=int,       help='Batch size')

	parser.add_argument("-gpu",		type=str,               default='3,4',			help="Set GPU Ids : Eg: For CPU = -1, For Single GPU = 0, For Multiple GPU = 0,1")
	...
	if not args.restore: args.name = args.name + str(args.max_epochs) +'_' + time.strftime("%d_%m_%Y") + '_' + time.strftime("%H:%M:%S")
