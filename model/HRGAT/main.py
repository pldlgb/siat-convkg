...
def parse_args():
	...
	# network arguments
	args.add_argument("-data", "--data", default= "../../data/PharmKG/", help="data directory")  #"../data/WN18RR/"
	args.add_argument("-e_g", "--epochs_gat", type=int, default=100, help="Number of epochs")  # 3600
	args.add_argument("-e_c", "--epochs_conv", type=int, default=25, help="Number of epochs")  # 200
	args.add_argument("-w_gat", "--weight_decay_gat", type=float, default=1e-5, help="L2 reglarization for gat")  # 5e-6
	...
	args.add_argument("-outfolder", "--output_folder", 
                   default="./checkpoints/PharmKG/out/", help="Folder name to save the models.")

	args.add_argument("-eval_type", "--eval_type", default='last')
	# arguments for GAT
	args.add_argument("-b_gat", "--batch_size_gat", type=int, default=800000, help="Batch size for GAT")  # 86835
	...
	args.add_argument("-margin", "--margin", type=float, default=1, help="Margin used in hinge loss")  # 5
	...
	args.add_argument("-o", "--out_channels", type=int, default=50, help="Number of output channels in conv layer")  #500
	args.add_argument("-drop_conv", "--drop_conv", type=float, default=0.3, help="Dropout probability for convolution layer")  # 0.0
...
def train_conv(args):
	...
	model_conv = SpKBGATConvOnly(entity_embeddings, relation_embeddings, args.entity_out_dim, args.entity_out_dim, args.drop_GAT, args.drop_conv, args.alpha, args.alpha_conv, args.nheads_GAT, args.out_channels)
...
def evaluate_conv(args, unique_entities, eval_type):
	...
	model_conv.load_state_dict(torch.load('{0}conv/trained_{1}.pth'.format(args.output_folder, args.epochs_conv - 1)))
